...
recoded_cart <- cart_MM %>% 
  ...
  mutate(CR_response_d_30 = case_when(
    str_detect(day_30_response_s_cr_cr_vgpr_pr_nr, "CR")   ~ "≥ CR",
    is.na(day_30_response_s_cr_cr_vgpr_pr_nr)              ~ NA_character_,
    TRUE                                                   ~ "< CR"
  )) %>% 
  ...
  mutate(CR_response_d_90 = case_when(
    str_detect(day_90_response_s_cr_cr_vgpr_pr, "CR")   ~ "≥ CR",
    is.na(day_90_response_s_cr_cr_vgpr_pr)              ~ NA_character_,
    TRUE                                                ~ "< CR"
  )) %>% 
  # Remove the < and > sign
  ...
  mutate(CR_response_d_90 = case_when(
    !is.na(CR_response_d_90)                                  ~ CR_response_d_90,
    evaluable_90days == "PD=death"                            ~ "< CR",
    evaluable_90days == "not evaluable"                       ~ NA_character_,
    orr_response_d_30_verified == "PD/SD/MR"                  ~ "< CR",
    !is.na(date_of_progression_documentation_if_any)          ~ "< CR",
    is.na(CR_response_d_90)                         ~ "< CR"
  )) 
```
...
recoded_cart %>% 
  select(
    ...
    CR_response_d_30,
    days_at_bleeding_post_cart,
    number_of_dvt,
    no_of_days_post_cart_at_time_of_stroke_arterial_clot,
    bleeding_y_n
  ) %>% 
  tbl_summary(by= bleeding_y_n,
              type = list(c(extramedullary_disease_1_0,
                            ...
                            any_CRS, any_ICANS)
                          ~ "categorical", 
                          c(days_at_bleeding_post_cart, no_of_days_post_cart_at_time_of_stroke_arterial_clot)
                          ~ "continuous"),
...
recoded_cart %>% 
  select(
    ...
    ferritin_pre_infusion = ferritin_day_6,
    ferritin_peak_ng_ml,
    crp_pre_leukapheresis,
    crp_pre_infusion = crp_day_6,
    crp_peak_ng_ml,
    ldh_pre_infusion = ldh_day_6,
    bleeding_y_n
  ) %>% 
  tbl_summary(by= bleeding_y_n,
...
<!-- ```{r glm} -->
<!-- tbl_uvregression(recoded_cart3[c("plt_apheresis", "r_iss_at_car_t_infsion",  -->
<!--                                  "ldh_at_pre_leukapheresis",  "history_of_bleeding_y_n", -->
<!--                                  "highest_gr_crs_grade_0_5", "ICANS_cat", -->
<!--                                  "bleeding_y_n")], -->
<!--                    method = glm,  -->
<!--                    y = bleeding_y_n, -->
