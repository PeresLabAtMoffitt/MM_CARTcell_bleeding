...
recoded_cart <- cart_MM %>% 
  ...
  mutate(crp_pre_leukapheresis = case_when(
    ...
    TRUE                                            ~ as.numeric(crp_pre_leukapheresis) ####### coalesce
    )) %>% 
...
recoded_cart %>% 
  select(
    age, sex,
    platelet_at_time_of_bleeding_event_1,
    ecog_at_screening,
...
# LDH <- 
  ...
  mutate(name = str_remove(name, "_k_ul|_k_u_l"),
         day = case_when(
           ...
           str_detect(name, "day")               ~ as.numeric(str_match(name, "(plt_day_)(.*)")[,3])
         ),
         type = str_match(name, "^([a-z]*)_")[,2]) %>% 
  group_by(bleeding_y_n, name, day) %>% 
